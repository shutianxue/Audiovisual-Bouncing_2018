function [position,dudt,fofthandle] = interparc(t,px,py,varargin)
% interparc: interpolate points along a curve in 2 or more dimensions
% Possible input format:
%   pt = interparc(t,px,py)              a 2-d curve
%   pt = interparc(t,px,py,pz)           a 3-d curve
% 	pt = interparc(t,px,py,pz,pw,...)    a 4-d or higher dimensional curve
% 	pt = interparc(t,px,py,method)       a 2-d curve, method is specified
% 	[pt,dudt,fofthandle] = interparc(t,px,py,...) eturns derivatives, and a function handle
%
% Description:
% Interpolates new points at any fractional point along the curve 
% defined by a list of points in 2 or more dimensions. 
% The curve may be defined by any sequence of non-replicated points.
%
%% =========== Inputs ===========
%  t   - vector of numbers, 0 <= t <= 1, 
%        Define the fractional distance along the curve to interpolate the curve at. 
%        t = 0: generate the very first point in the point list, 
%        t = 1: yields the last point in that list.
%        t = 0.5: yield the mid-point on the curve in terms of arc length as the
%        curve is interpolated by a parametric spline.
%
%        If t is a scalar integer, at least 2, then it specifies the number of equally spaced
%        points in arclength to be generated along the curve.
%
%  px, py, pz, ... - vectors of length n (n must be at least 2)
%        Define points along the curve. 
%        Exact Replicate points should not be present in the curve, 
%        although there is no constraint that the curve has replicate independent variables.
%
%  method - (OPTIONAL) string flag - 
%        denotes the method used to compute the points along the curve.
%
%        method may be any of 'linear', 'spline', or 'pchip',
%        or any simple contraction thereof, such as 'lin','sp', or even 'p'.
%        
%        'linear' --> Uses a linear chordal approximation to interpolate the curve.
%                     This method is the most efficient.
%
%        'pchip' --> Uses a parametric pchip approximation for the interpolation in arc length.
%
%        'spline' --> (default) Uses a parametric spline approximation for the interpolation in arc length. Generally for a smooth curve,
%                     This method is the most accurat.

%        'csape' --> if available, this tool will allow a periodic spline fit for closed curves.
%               ONLY use this method if your points should represent a closed curve.
%               
%               If the last point is NOT the same as the first point on the curve, then the curve
%               will be forced to be periodic by this option. That is, the first point will be replicated onto the end.
%               If csape is not present in your matlab release, then an error will result.
%
%% =========== Outputs ===========
%  position - Interpolated points at the specified fractional distance (in arc length) along the curve.
%
%  dudt - when a second return argument is required, interparc will return the parametric derivatives
...
%% =========== Example ===========
% Example 1:
% % Interpolate a set of unequally spaced points around the perimeter of a unit circle, 
% % generating equally spaced points around the perimeter.
%{
...
%
% Example 2:
% % For the previous set of points, generate exactly 6 points around the parametric splines, verifying
% % the uniformity of the arc length interpolant.
% pt = interparc(6,px,py,'spline');
%
% % Convert back to polar form. See that the radius is indeed 1, quite accurately.
...
% %        1.2565
%
% % Six points around the circle should be separated by 2*pi/5 radians, if they were perfectly uniform. 
% % The slight differences are due to the imperfect accuracy of the parametric splines.
...
if nargin > 3
  ...
  % anything that remains in varargin must add an additional dimension on the curve/polygon
...
position = NaN(nt,ndim);

% Compute the chordal (linear) arclength of each segment. 
% This step is be needed for all methods.
chordLengh = sqrt(sum(diff(pxy,[],1).^2,2));

% Normalize the arclengths to a unit total
chordLengh = chordLengh/sum(chordLengh);

% cumulative arclength
cumArc = [0;cumsum(chordLengh)];

% The linear interpolant is trivial. do it as a special case
if method(1) == 'l'
  % The linear method.
  
  % determine which interval did each point fall in, in terms of t?
  [binSum,binAccount] = histc(t,cumArc); 
  
  % catch any problems at the ends
  binAccount((binAccount <= 0) | (t <= 0)) = 1;
  binAccount((binAccount >= n) | (t >= 1)) = n - 1;
  
  %% interpolate
  s = (t - cumArc(binAccount))./chordLengh(binAccount);
  % allow the code to work on older releases that don't have bsxfun
  position = pxy(binAccount,:) + (pxy(binAccount+1,:) - pxy(binAccount,:)).*repmat(s,1,ndim);
  
  % do we need to compute derivatives here?
  if nargout > 1
    dudt = (pxy(binAccount+1,:) - pxy(binAccount,:))./repmat(chordLengh(binAccount),1,ndim);
  end
...
%%
% If we drop down to here, we have either a spline or csape or pchip interpolant to work with.
...
% catch the case where there were exactly three points in the curve, 
% and spline was used to generate the interpolant. 
% In this case, spline creates a curve with only one piece, not two.
if (numel(cumArc) == 3) && (method(1) == 's')
  cumArc = spl{1}.breaks;
  n = numel(cumArc);
  chordLengh = sum(chordLengh);
end
...
for i = 1:spl{1}.pieces
  ...
  [tout,yout] = ode45(@(t,y) segkernel(t,y),[0,chordLengh(i)],0,opts); %#ok
  seglen(i) = yout(end);
end
...
[binSum,binAccount] = histc(t*totalsplinelength,cumseglen); %#ok
...
for i = 1:nt
  ...
  si = s(i) - cumseglen(binAccount(i));
  
  % extract polynomials for the derivatives
  % in the interval the point lies in
  for j = 1:ndim
    polyarray(j,:) = spld{j}.coefs(binAccount(i),:);
  end
  ...
  if ~isempty(te)
    ...
    if abs(yout(1)) < abs(yout(end))
      ...
      ti(i) = tout(1) + cumArc(binAccount(i));
    else
      % the event must have been at the end.
      ti(i) = tout(end) + cumArc(binAccount(i));
    end
  end
end
